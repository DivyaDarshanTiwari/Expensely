...
const AddTransaction = () => {
  ...
  const [transactionData, setTransactionData] = useState({
    ...
    paidBy: null, // For expenses
    receivedBy: null, // For income
    splitType: "equal",
    splitDetails: {},
  });
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        ...
        const currentUser = res.data.find((member: { id: number; }) => member.id === 1);
        if (currentUser) {
          if (transactionType === 'expense') {
            setTransactionData(prev => ({ ...prev, paidBy: currentUser }));
          } else {
            setTransactionData(prev => ({ ...prev, receivedBy: currentUser }));
          }
        }
        
        // For expenses, select all members by default
        if (transactionType === 'expense') {
  ...
  const selectedCategory = categories.find(cat => cat.id === transactionData.category);
  ...
  const handleSubmit = async () => {
    ...
    if (transactionType === 'expense') {
      ...
      if (!transactionData.receivedBy) {
        Alert.alert("Error", "Please select who received the income");
        return;
      }
    }
    ...
    try {
      ...
      if (transactionType === 'expense') {
        payload.paidBy = transactionData.paidBy.id;
        payload.splitWith = selectedMembers.map(member => member.id);
        payload.splitType = transactionData.splitType;
        payload.splitDetails = transactionData.splitDetails;
      } else {
        payload.receivedBy = transactionData.receivedBy.id;
      }

      const endpoint = transactionType === 'expense' 
        ? "http://localhost:8082/api/v1/expenses/create"
        : "http://localhost:8082/api/v1/income/create";
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[
                  styles.categoryItem,
                  transactionData.category === item.id && styles.categoryItemSelected
                ]}
                onPress={() => {
                  setTransactionData(prev => ({ ...prev, category: item.id }));
      ...
      <View style={styles.modalOverlay}>
        <Animated.View style={[styles.modalContent, { transform: [{ scale: formScale }] }]}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              const isSelected = transactionType === 'expense' 
                ? transactionData.paidBy?.id === item.id
                : transactionData.receivedBy?.id === item.id;
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (transactionType === 'expense') {
                      setTransactionData(prev => ({ ...prev, paidBy: item }));
                    } else {
                      setTransactionData(prev => ({ ...prev, receivedBy: item }));
                    }
        ...
        >
          ...
          >
            ...
            <View style={styles.amountInputContainer}>
              <Text style={[styles.currencySymbol, { color: getAmountColor() }]}>
                {getAmountPrefix()}
              </Text>
              <TextInput
        ...
        >
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput
              ...
              onChangeText={(text) => 
                setTransactionData(prev => ({ ...prev, date: text }))
              }
            />
          </View>
          ...
          <View style={styles.inputContainer}>
            ...
            >
              <View style={styles.selectContent}>
                {(transactionType === 'expense' ? transactionData.paidBy : transactionData.receivedBy) ? (
                  <>
                    <View style={styles.memberAvatar}>
                      <Text style={styles.memberInitial}>
                        {(transactionType === 'expense' ? transactionData.paidBy : transactionData.receivedBy).name.charAt(0).toUpperCase()}
                      </Text>
                    </View>
                    <Text style={styles.selectText}>
                      {(transactionType === 'expense' ? transactionData.paidBy : transactionData.receivedBy).name}
                    </Text>
                  </>
                ) : (
                  <Text style={[styles.selectText, { color: "#9CA3AF" }]}>
                    {transactionType === 'expense' ? 'Select who paid' : 'Select who received'}
                  </Text>
                )}
              </View>
              <Ionicons name="chevron-down" size={20} color="#6B7280" />
            ...
            <View style={styles.inputContainer}>
              ...
              <View style={styles.splitInfo}>
                <Text style={styles.splitText}>
                  ...
                  ${transactionData.amount ? (parseFloat(transactionData.amount) / selectedMembers.length).toFixed(2) : '0.00'} each
                </Text>
